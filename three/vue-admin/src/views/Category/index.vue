	...
	<div class="root">
		<el-tree :data="treeData" :expand-on-click-node="false" default-expand-all>
			<div class="custom-tree-node" slot-scope="{ node,data }">
				...
				<div class="right">
					<!-- 三级分类没有新增图标 -->
					<i class="el-icon-plus" v-if="data.id ===0 || data.fid === 0 || !data.avatar" @click="beginAdd(data)"></i>
					<!-- 根节点没有修改图标 -->
					<i class="el-icon-edit-outline" v-if="data.id !== 0" @click="beginUpdate(data)"></i>
					<!-- 只有三级分类才有删除图标 -->
		...
		<el-dialog
			...
			<el-form :model="edit.model" label-width="100px">
				<el-form-item label="父级分类：">
					ROOT
					<span style="margin: 0 10px">-</span>
					<!-- 一级分类 -->
					<el-select v-model="edit.select.main.value" v-show="edit.select.main.isShow" :disabled="edit.select.main.disabled">
						<el-option v-for="item in mainList" :key="item.id" :label="item.name" :value="item.id"></el-option>
					</el-select>
					<span style="margin: 0 10px">-</span>
					<!-- 二级分类 -->
					<el-select v-model="edit.select.sub.value" v-show="edit.select.sub.isShow" :disabled="edit.select.sub.disabled">
...
<script type="text/ecmascript-6">
	...
	let { mapState, mapActions } = createNamespacedHelpers("category");
		...
		name: "Category",
	        ...
	        computed: {
		        ...mapState(["list"]),          // 将category仓库中的list映射到当前组件的计算属性中
		        treeData() {
	        ...
	        methods: {
		        ...
		        _updateChildNode(node) {                        // 你给我一个对象节点我就帮你把它的children构造好
		        ...
		        beginAdd(node) {
                        ...
                        beginUpdate(node) {
                                if(node.fid === 0) {                                    // 修改的是一级分类
                                        this.edit.select.main.isShow = false;
                                        this.edit.select.sub.isShow = false;
                                        this.edit.level = 1;
                                } else if(!node.avatar) {                               // 修改的是二级分类
                                        this.edit.select.main.isShow = true;
		        ...
		        uploadRemoveHandler(file, fileList) { this.edit.model.newAvatar = null; },
		        save() {
				if(this.edit.mode) {
                ...
                watch: {
                        'edit.select.main.value'(newValue,oldValue) {
                                if(!this.edit.mode && this.edit.level ===3) {   // 如果是修改，并且修改的是三级分类
                                        this.edit.select.sub.value = this.subList.length > 0 ? this.subList[0].id : "";
                                }
                                if(!this.edit.mode && this.edit.level === 2) {  // 如果是修改，并且修改的是二级分类
                                        this.edit.model.fid = newValue;
                                }
                        },
                        'edit.select.sub.value'(newValue,oldValue) {
                                if(!this.edit.mode && this.edit.level ===3) {   // 如果是修改，且修改的是三级分类
                                        this.edit.model.fid = newValue;
                                }
                        },
...
<style scoped>
	...
	.custom-tree-node>.right>* {margin: 0 10px; }
	.avatar-wrapper{
		display: flex;
	}
	.old-avatar-wrapper{
	...
	.old-avatar-wrapper>.old-avatar{
		width: 148px;
	}
	.old-avatar-wrapper>span{
		display: block;
		text-align: center;
	}
</style>